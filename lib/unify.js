...
async function processDataElement (identifier, instance, validator, emitter, options) {
  const validation = validator(instance);
  if (validation.valid && options.annotations) {
  ...
  fetch(METASCHEMA).then(async (dialect) => {
    ...
    if (Array.isArray(unify.dataset)) {
      for (const data of unify.dataset) {
        await processDataElement(unify.$id, data, validator, emitter, options);
      }
